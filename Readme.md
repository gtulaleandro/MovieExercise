...
### Resolution
# Running the Spring Boot project using Docker
...
We can expose another port if we want.

# Retries against API

If, for some reason, we can't get access to the final result, then the spring application will retry 2 times, using
exponential backoff configuration. If no success, then an exception will be thrown.

# Test coverage
These solutions contain unit test for each layer. 
